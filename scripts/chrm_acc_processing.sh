...
# Variables
data_path=../data/chrm_acc_feature/"$HISTONE_NAME"
...
if [ ! -f "$data_path"/"$HISTONE_NAME"_sorted_chrm_acc_marks.bed ]; then
    ...
    sort -k1,1 -k2,2n -k3,3n > "$data_path"/"$HISTONE_NAME"_sorted_chrm_acc_marks.bed
fi


# Perform interval intersection using BEDTools
bedtools intersect -a "$data_path"/"$HISTONE_NAME"_sorted_chrm_acc_marks.bed -b "$sorted_regions_file_bed" -wo > overlaps_"$HISTONE_NAME"_"$OUTPUT_NAME".bed
